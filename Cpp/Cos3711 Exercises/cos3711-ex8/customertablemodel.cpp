...
CustomerTableModel* CustomerTableModel::getInstance()
{
    static CustomerTableModel *instance = nullptr;
...
bool CustomerTableModel::removeRow(int row)
{
    beginRemoveRows(QModelIndex(), row, row);
    delete customerList.at(row);
...
QList<Customer*>& CustomerTableModel::getList()
{
    return customerList;
}

void CustomerTableModel::setList(QList<Customer*> list)
{
    // Clear customer list
    clear();
    ...
    for (int count = 0; count <list.count(); count++) {
        ...
        copyCustomer = cf.createCustomer(className,
                                         list.at(count)->getTitle(),
                                         list.at(count)->getName(),
                                         list.at(count)->getAddress(),
                                         list.at(count)->getPostCode(),
                                         list.at(count)->getCode(),
                                         list.at(count)->getBalance());
        // Do not append null object
        if (copyCustomer != nullptr) {
           addCustomer(copyCustomer);
        }
    }
}
